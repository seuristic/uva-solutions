...
2. Solution files sometimes land at the repo root instead of `solutions/`. Move any
   root-level `pNNNNN.cpp` into `solutions/` before staging (scratch files stay put).
3. For every `solutions/pNNNNN.cpp` with no row in the `README.md` table — not just
